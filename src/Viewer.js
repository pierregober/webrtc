...
async function startViewer(
  ...
  const getIceServerConfigResponse = await kinesisVideoSignalingChannelsClient
    ...
    .promise();
  const iceServers = [];
  if (!props.natTraversalDisabled && !props.forceTURN) {
    iceServers.push({
      urls: `stun:stun.kinesisvideo.${props.region}.amazonaws.com:443`,
    });
  }
  if (!props.natTraversalDisabled) {
  ...
  const configuration = {
    iceServers,
    iceTransportPolicy: props.forceTURN ? "relay" : "all",
  };
  viewer.peerConnection = new RTCPeerConnection(configuration);
  if (props.openDataChannel) {
